...
( SELECT
  ...
  ( SELECT                     /* Modification section */
      ...
      'AUTO' ENVIRONMENT,      /* AUTO for automatic determination based on current environment,
                                  ABAP, BPC, BW, CRM, ERP, ESS, EWM, FRA, HIST, IQ, LCACHE, MDCSYS, MDCTEN, MULTI, PORTAL, PROD, SCM, SDA, SINGLE, SRM, SSS, SYSREP and comma separated combinations */
...
( SELECT
  ...
  ( SELECT 'ALL' ENV,     ' ' FILE_NAME,          ' ' SECTION,                ' ' PARAMETER_NAME,                          ' ' NOT_1, ' ' MIN_VALUE_1,      ' ' MAX_VALUE_1,         ' ' MIN_VALUE_2,           ' ' MAX_VALUE_2,           -1 MIN_REV, -1 MAX_REV, -1 MIN_VERS, -1 MAX_VERS, '' SAP_NOTE, ' ' AREA, ' 'TYPE FROM DUMMY WHERE 1 = 0 UNION ALL
    ...
    SELECT 'ALL',         'daemon.ini',           'xsengine.%',               '%',                                         ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1,        '', ' ',              'D' FROM DUMMY UNION ALL
    SELECT 'ALL',         'daemon.ini',           'dpserver.%',               '%',                                         ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1,        '', ' ',              'D' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'global.ini',           'auditing configuration',   'global_auditing_state',                     ' ', 'true',                     '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 93,    -1,  1,  2,  '863362', 'security',       ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'global.ini',           'execution',                'max_concurrency_dyn_min_pct',               ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2222250', 'CPU',            ' ' FROM DUMMY UNION ALL
    SELECT 'ALL',         'global.ini',           'execution',                'max_concurrency_hint',                      ' ', '-- SPECIAL --',            '-- SPECIAL --',           '-- no recommendation --', '-- no recommendation --', -1,     2, -1, 29, '2222250', 'CPU',            ' ' FROM DUMMY UNION ALL
    SELECT 'ALL',         'global.ini',           'execution',                'max_concurrency_hint_dyn_min_pct',          ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2222250', 'CPU',            ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'MULTI',       'global.ini',           'internal_hostname_resolution', '<ip_address>',                          ' ', '<host_name>',              '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 70,    -1,  1, -1, '2183363', ' ',              ' ' FROM DUMMY UNION ALL
    SELECT 'SINGLE',      'global.ini',           'internal_hostname_resolution', '%',                                     ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1,        '', ' ',              'A' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'global.ini',           'memorymanager',            'oom_dump_time_delta',                       ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '1999997', 'memory',         'A' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'global.ini',           'multidb',                  'singletenant',                              ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2101244', 'MDC',            'D' FROM DUMMY UNION ALL
    SELECT 'ALL',         'global.ini',           'multidb',                  'database_isolation',                        ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2269429', 'MDC',            'D' FROM DUMMY UNION ALL
    SELECT 'ALL',         'global.ini',           'multidb',                  'mode',                                      ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2101244', 'MDC',            'D' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'global.ini',           'persistence',              'savepoint_pre_critical_flush_retry_threshold',' ','-- SPECIAL --',           '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1, 12.02,  2,  2, '2538561', 'disk I/O',       ' ' FROM DUMMY UNION ALL
    SELECT 'ALL',         'global.ini',           'persistence',              'savepoint_pre_critical_flush_retry_threshold',' ','-- SPECIAL --',           '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 20,    21,  2,  2, '2538561', 'disk I/O',       ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'SYSREP',      'global.ini',           'system_replication',    'enable_cache_pages_for_preload_version_check', ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2587465', 'system rep.',    ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'SYSREP',      'global.ini',           'system_replication',       'logshipping_secondary_timeout',             ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2400007', 'system rep.',    ' ' FROM DUMMY UNION ALL
    SELECT 'SYSREP',      'global.ini',           'system_replication',       'logshipping_snapshot_logsize_threshold',    ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2332284', 'system rep.',    ' ' FROM DUMMY UNION ALL
    SELECT 'SYSREP',      'global.ini',           'system_replication',       'logshipping_snapshot_min_time_interval',    ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2332284', 'system rep.',    ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'BPC',         'global.ini',           'table_placement',          'method',                                    ' ', '2',                        '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1,  1,  1, '2003863', 'performance',    ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'BW',          'global.ini',           'table_placement',          'same_num_partitions',                       ' ', 'true',                     '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1,  1,  1, '1908075, 2003863, 2334091', 'performance',    ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'admission_control_events', 'record_limit',                              ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2088971', 'data volume',    ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'communication',            'maxendpoints',                              ' ', '20000',                    '40000',                   '-- no recommendation --', '-- no recommendation --',100,122.14,  1,  1, '2382421', 'terminations',   ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'communication',            'maxidleage',                                ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2222200', 'performance',    ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'communication',            'signal_recv_timeout',                       ' ', '60000',                    '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,     1,  2,  2, '2427296', 'terminations',   ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'communication',            'signal_send_timeout',                       ' ', '60000',                    '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,     1,  2,  2, '2427296', 'terminations',   ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'csaccessor',               'disable_table_consistency_checks',          ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2603049', 'consistency',    ' ' FROM DUMMY UNION ALL
    SELECT 'ALL',         'indexserver.ini',      'delta',                    'btree_cache_aggressiveness',                ' ', '0',                        '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,112.03,  1,  1, '2338166', 'wrong results',  ' ' FROM DUMMY UNION ALL
    SELECT 'ALL',         'indexserver.ini',      'delta',                    'btree_cache_aggressiveness',                ' ', '0',                        '-- no recommendation --', '-- no recommendation --', '-- no recommendation --',120,   121,  1,  1, '2338166', 'wrong results',  ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'dictstore',                'check_main_dictionary_after_load',          ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2116157', 'consistency',    ' ' FROM DUMMY UNION ALL
    SELECT 'ALL',         'indexserver.ini',      'dictstore',                'check_main_dictionary_after_load_repair',   ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2116157', 'consistency',    ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'distribution',             'systemreplication_protocol_version',        ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2587258', 'hang situation', ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'BW',          'indexserver.ini',      'dso',                      'search_realization',                        ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2613469', 'crash',          ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'global',                   'timezone_default_data_schema_name',         ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2100040', 'performance',    ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'joins',                    'map_threshold_factor_uns_mapping',          ' ', '4000000000',               '-- no recommendation --', '-- no recommendation --', '-- no recommendation --',122.15,122.15,1, 1, '2599767', 'crash',          ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'livecache',                'max_version_retention_time',                ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2169283', 'liveCache',      ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'PROD',        'indexserver.ini',      'performance_analyzer',     'planviz_enable',                            ' ', 'false',                    '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    23,  2,  2, '2542972', 'memory leak',    ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'persistence',              'runtimedump_for_blocked_savepoint_timeout', ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2100009', 'trace',          ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'planningengine',           'formula_parallel_limit',                    ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2607741', 'wrong results',  ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'repository',               'sqlscript_mode',                            ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '1994104', ' ',              ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'search',                   'collect_top_documents_count',               ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2611281', 'terminations',   ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'search',                   'qo_extensive_checks',                       ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2616802', 'terminations',   ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'SDA',         'indexserver.ini',      'smart_data_access',        'enable_subqueries_gen',                     ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2609173', 'termination',    ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'sql',                      'esx_level',                                 ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2599949', 'crash',          ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'sql',                      'nested_trigger_limit',                      ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2130431', 'terminations',   ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'statisticsserver',         'initial_profile',                           ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2127247', 'stat. server',   'D' FROM DUMMY UNION ALL
    ...
    SELECT 'ALL',         'indexserver.ini',      'transaction',              'aggressive_gc_interval',                    ' ', '300',                      '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1,  1,  1, '2169283', 'performance',    ' ' FROM DUMMY UNION ALL
    ...
    SELECT 'ESS',         'statisticsserver.ini', 'statisticsserver_general', 'active',                                    ' ', '-- no recommendation --',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', -1,    -1, -1, -1, '2147247', 'monitoring',     'D' FROM DUMMY UNION ALL
...
SELECT 'ALL','indexserver.ini','search','qo_top_1_optimization',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,-1,1,1,'2238679','CPU;MVCC;top_optimization;internal_parameter',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','delta','btree_cache_aggressiveness',' ','0','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,121.999,1,1,'2339989','consistency',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','row_engine','garbage_lob_file_handler_enabled',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.019,1,1,'2351467','GC;RowStore;memory_leak;LOB',' ' from DUMMY UNION ALL
SELECT 'SYSREP','global.ini','system_replication','enable_log_retention',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,-1,1,1,'2372278','system_replication',' ' from DUMMY UNION ALL
SELECT 'SYSREP','global.ini','system_replication','logshipping_replay_push_callback_count',' ','300000','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.049,1,1,'2381431','system_replication;log_replay',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','session','fda_enable',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.039,1,1,'2406964','FDA;indexserver_crash',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','session','max_lob_prefetch_size',' ','2147483647','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,-1,1,1,'2408032','LOB;JDBC',' ' from DUMMY UNION ALL
SELECT 'SDA','indexserver.ini','smart_data_access','enable_binary_transfer',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,-1,1,1,'2416236','SDA',' ' from DUMMY UNION ALL
SELECT 'BW','indexserver.ini','indexing','use_sparsity_check',' ','no','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.089,1,1,'2430257','BWoH;SCM;Memory_leak;compression;delta_merge',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','resource_tracking','service_thread_sampling_monitor_enabled',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',122.07,122.079,1,1,'2437287','indexserver_crash;system_view',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','row_engine','page_compaction_max_pages',' ','2097152','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.079,1,1,'2440717','indexserver_crash;rowstore_reorg',' ' from DUMMY UNION ALL
SELECT 'SYSREP','global.ini','system_replication','enable_log_retention',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.049,1,1,'2442450','system_replication;log_replay',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','sql','disjunctive_hash_join_enabled',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.049,1,1,'2445356','indexserver_crash;join_engine;optimizer',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','authentication','last_successful_connect_update_interval',' ','300','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.099,1,1,'2460123','CPU;lock_manager',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','row_engine','dynamic_parallel_insert_max_workers',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.089,1,1,'2461264','indexserver_crash;Rowstore',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','sql','disjunctive_hash_join_enabled',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.089,1,1,'2462138','indexserver_crash',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','communication','ssl',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.089,1,1,'2464452','SSL;indexserver_crash;encryption',' ' from DUMMY UNION ALL
SELECT 'SDA','indexserver.ini','smart_data_access','enable_binary_transfer',' ','-- no recommendation --','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.079,1,1,'2472882','virtual_table;LOB;SDA',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','calcengine','gc_cleanup_retention_sec',' ','604800','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.109,1,1,'2474345','calc_engine;scale-out;GC;performance',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','calcengine','instantiate_max_itab_transformation_size',' ','<number_higher_than_number_of_predicates>','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.099,1,1,'2476064','calculation_view',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','calcengine','optimize_simplify_filter_enabled',' ','0','-- no recommendation --','-- no recommendation --','-- no recommendation --',122.04,122.099,1,1,'2476216','indexserver_crash;calc_engine',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','lobhandling','garbage_collect_interval_s',' ','0','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.099,1,1,'2477482','partitioning;LOB',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','resource_tracking','service_thread_sampling_monitor_include_passport',' ','no','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.119,1,1,'2478767','system_view',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','session','fda_enable',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.109,1,1,'2495247','FDA;wrong_result',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','distribution','client_distibution_mode',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',121,122.119,1,1,'2500745','scale-out;LOB;repository;deploy',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','joins','disable_aggregate_simplification',' ','true','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.119,1,1,'2509750','partitioning',' ' from DUMMY UNION ALL
SELECT 'ALL','nameserver.ini','session','max_number_of_sessions_per_partition',' ','2048','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,-1,1,1,'2516320','connection_limit;session',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','optimize_compression','singleindex_consider_for_compressed_columns',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',122.1,122.119,1,1,'2516807','indexes;CPU;join_engine',' ' from DUMMY UNION ALL
SELECT 'BW','indexserver.ini','calcengine','ce2qo_enable_boundary_op',' ','0','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.139,1,1,'2517443','performance; calc_view;Bex;memory_leak;FEMS;BWoH',' ' from DUMMY UNION ALL
SELECT 'SYSREP','global.ini','system_replication','enable_evict_pages_after_takeover',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',122.1,122.119,1,1,'2519008','indexserver_crash;system_replication;page_cache',' ' from DUMMY UNION ALL
SELECT 'SYSREP','indexserver.ini','row_engine','keep_shared_memory_over_restart',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.119,1,1,'2522580','indexserver_crash; system_replication;RowStore;delta_data_shipping',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','communication','ssl',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.119,1,1,'2523100','SSL;indexserver_crash;encryption',' ' from DUMMY UNION ALL
SELECT 'ERP','indexserver.ini','sql','plan_cache_size',' ','5368709120','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,-1,1,1,'2523230','BSoH;error_2048;sql_plan_cache',' ' from DUMMY UNION ALL
SELECT 'SYSREP','global.ini','system_replication','enable_log_retention',' ','-- no recommendation --','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.019,1,1,'2525420','system_replication',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','session','fda_enable',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.129,1,1,'2534365','indexserver_crash;FDA',' ' from DUMMY UNION ALL
SELECT 'BW','indexserver.ini','parallel','max_lock_retry',' ','60','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,-1,1,1,'2535656','OLAP:BWoH;SCM;lock_manager',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','query_mediator','filtertransport_enable',' ','0','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.129,1,1,'2535996','indexserver_crash;olap_engine;optimizer',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','search','late_materialization_threshold_for_insert',' ','10000000000','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.119,1,1,'2539459','indexserver_crash;trex;late_materialization',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','session','protocol_trace',' ','-- no recommendation --','-- no recommendation --','-- no recommendation --','-- no recommendation --',122.11,122.139,1,1,'2540710','indexserver_crash;PowerPC;trace',' ' from DUMMY UNION ALL
SELECT 'SDA','indexserver.ini','smart_data_access','join_relocation',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,-1,1,1,'2542329','SDA;virtual_table',' ' from DUMMY UNION ALL
SELECT 'SYSREP','global.ini','system_replication','preload_column_tables',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.129,1,1,'2542853','indexserver_crash;system_replication;takeover;preload;delta_data_shipping;upgrade',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','performance_analyzer','planviz_enable',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.139,1,1,'2542972','indexserver_crash;planviz',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','calcengine','ce2qo_disable_unfolding',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.129,1,1,'2544123','indexserver_crash;calc_engine;optimizer',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','row_engine','dynamic_parallel_insert_max_workers',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.129,1,1,'2544949','indexserver_crash;redistribution;scale-out',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','persistence','undo_file_cache_size',' ','512','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.119,1,1,'2547543','CPU;performance;undo_files',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','session','fda_enable',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.109,1,1,'2547605','indexserver_crash',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','series','series_column_lrle_compression_enabled',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.139,1,1,'2549632','compression_LRLE',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','query_mediator','cache_enable',' ','0','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.139,1,1,'2555376','indexserver_crash;scale-out;sql_plan_cache',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','calcengine','disabled_patterns',' ','23','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.109,1,1,'2557609','calculation_view;calc_engine',' ' from DUMMY UNION ALL
SELECT 'ALL','xsengine.ini','sql','use_new_placement_scheme_for_replicas',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.139,1,1,'2558683','XS_engine',' ' from DUMMY UNION ALL
SELECT 'SDI','dpserver.ini','persistence','receiver_comi_interval_sec',' ','2','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.139,1,1,'2560056','SDI',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','persistence','savepoint_max_pre_critical_flush_duration',' ','-- no recommendation --','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.109,1,1,'2565285','performance;GC;LOB;MVCC',' ' from DUMMY UNION ALL
SELECT 'SDA','indexserver.ini','smart_data_access','enable_binary_transfer',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.149,1,1,'2566685','SDA;virtual_table',' ' from DUMMY UNION ALL
SELECT 'SYSREP','global.ini','pitrestart','replay_step_size',' ','88080384','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.139,1,1,'2567255','indexserver_crash;system_replication;takeover',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','calcengine','ce2qo_skip_cache_lookup',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.089,1,1,'2573582','calculation_view;calc_engine',' ' from DUMMY UNION ALL
SELECT 'SYSREP','global.ini','system_replication','enable_log_retention',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.059,1,1,'2576669','system_replication;log_replay',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','backup','parallel_data_backup_backint_channels',' ','-- no recommendation --','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.159,1,1,'2579371','indexserver_crash;nameserver_crash;backup,backint',' ' from DUMMY UNION ALL
SELECT 'SYSREP','nameserver.ini','local_watchdog','interval',' ','0','-- no recommendation --','-- no recommendation --','-- no recommendation --',122.1,122.129,1,1,'2581373','system_replication;takeover',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','expensive_statement','internal',' ','-- no recommendation --','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.149,1,1,'2583885','indexserver_crash;trace',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','calcengine','disabled_patterns',' ','15','-- no recommendation --','-- no recommendation --','-- no recommendation --',122.06,122.069,1,1,'2583936','calculation_view;calc_engine;performance',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','calcengine','disabled_patterns',' ','43','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.149,1,1,'2584492','JDBC;Composite_Provider;calc_engine',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','resource_tracking','memory_tracking',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.149,1,1,'2586437','indexserver_crash;OOM;',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','row_engine','bulk_deletion_enabled',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.089,1,1,'2593025','GC;RowStore;indexserver_crash',' ' from DUMMY UNION ALL
SELECT 'SDA','indexserver.ini','smart_data_access','join_relocation',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,-1,1,1,'2593695','SDA;virtual_table;JDBC',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','sql','conservative_cs_anti_join_estimation_enabled',' ','true','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.119,1,1,'2000002','performance;join_engine',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','joins','disable_aggregate_simplification',' ','true','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.069,1,1,'2092196','join_engine;optimizer',' ' from DUMMY UNION ALL
SELECT 'BW','indexserver.ini','calcengine','disabled_patterns',' ','38','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.129,1,1,'2100040','BWoH;CPU;performance',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','row_engine','garbage_lob_file_handler_enabled',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',122.07,122.109,1,1,'2413261','LOB;GC',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','row_engine','garbage_lob_file_handler_enabled',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.049,1,1,'2413261','LOB;GC',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','sql','calc_view_with_dyn_ap_unfold',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,-1,1,1,'2441054','calculation_view',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','calcengine','ce2qo_transient_plan_for_data_var',' ','0','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,-1,1,1,'2441054','calculation_view',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','communication','SSL',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.129,1,1,'2547142','OOM;SSL;encryption',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','performance_analyzer','planviz_enable',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.149,1,1,'2575166','indexserver_crash;planviz',' ' from DUMMY UNION ALL
SELECT 'SDA','indexserver.ini','smart_data_access','join_relocation',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.149,1,1,'2585974','SDA;virtual_table;optimizer;Composite_provider',' ' from DUMMY UNION ALL
SELECT 'SDA','indexserver.ini','smart_data_access','join_relocation',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.149,1,1,'2587067','SDA;virtual_table',' ' from DUMMY UNION ALL
SELECT 'SYSREP','indexserver.ini','persistence','log_segment_size_mb',' ','1024','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.149,1,1,'2572012','system_replication',' ' from DUMMY UNION ALL
SELECT 'SYSREP','global.ini','system_replication','enable_cache_pages_for_preload_version_check',' ','true','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.099,1,1,'2587465','system_replication;delta_data_shipping',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','distribution','client_distibution_mode',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.159,1,1,'2591281','JDBC;lock_manager',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','sql','reload_tables',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.149,1,1,'2598453','indexserver_crash;transaction;MVCC',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','joins','map_threshold_factor_uns_mapping',' ','4000000000','-- no recommendation --','-- no recommendation --','-- no recommendation --',122.15,122.159,1,1,'2599767','indexserver_crash;join_engine;optimizer',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','joins','disable_aggregate_simplification',' ','true','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.149,1,1,'2583884','attribute_engine;trace',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','joins','prune_cached_plans',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.039,1,1,'2387680','partitioning;wrong_result',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','communication','signal_send_timeout',' ','60000','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.069,1,1,'2427296','network;trace',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','communication','signal_recv_timeout',' ','60000','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.069,1,1,'2427296','network;trace',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','persistence','continuous_flush_interval_s',' ','0','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.039,1,1,'2370160','consistency',' ' from DUMMY UNION ALL
SELECT 'BW','indexserver.ini','dso','activation_columnset_size',' ','4000000000','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.039,1,1,'2385658','BWoH;SCM;consistency;wrong_result',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','delta','btree_cache_aggressiveness',' ','0','-- no recommendation --','-- no recommendation --','-- no recommendation --',121,121.999,1,1,'2338166','duplicate_key',' ' from DUMMY UNION ALL
SELECT 'BW','indexserver.ini','olap','optimize_value_id_search',' ','no','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,120.999,1,1,'2321910','BWoH;BEX;OLAP_engine;FEMS',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','expensive_statement','use_in_memory_tracing',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,-1,1,1,'2608324','trace;',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','calcengine','disabled_patterns',' ','23','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.159,1,1,'2601405','calc_engine;calc_view',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','communication','default_read_timeout',' ','-1','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.109,1,1,'2488737','network;TREX;tcp_timeout',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','planningengine','formula_parallel_limit',' ','<large_integer_value>','-- no recommendation --','-- no recommendation --','-- no recommendation --',122.13,122.159,1,1,'2607741','wrong_result;planning_engine',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','search','collect_top_documents_count',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.159,1,1,'2611281','attribute_engine;',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','sql','force_block_fallback',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.159,1,1,'2614277','indexserver_crash;replicated_table',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','mds','cube_parallel_processing_11_job_size',' ','1000000000','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,120.999,1,1,'2321714','indexserver_crash;MDS',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','search','qo_extensive_checks',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,122.159,1,1,'2616802','indexserver_crash;CPU',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','joins','single_thread_execution_for_partitioned_tables',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,-1,1,1,'2620310','join_engine;performance;delta_merge',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','sqlscript','typecheck_procedure_input_param',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',120,-1,1,1,'2457876','calc_engine;calc_view',' ' from DUMMY UNION ALL
